...
\begin{figure}[ht]
\centering
\includegraphics[width=0.3\textwidth]{./assets/membranes.png}
...
Changes in the local environment of the material with substitution of GeO$_{2}$ into Ta$_{2}$O$_{5}$ are reflected by a shift in PDF peak positions (Figure \ref{fig:pdf}). Unfortunately, the resolution of the PDF limits our ability to properly attribute PDF peaks to Ta$_{2}$O$_{5}$ and GeO$_{2}$. For example, we expect a peak at around 1.73 \AA\;for the Ge-O pair \cite{STONE2001769} in the PDF pattern at 0.34 Ta cation fraction (Figure \ref{fig:pdf}), but we can only observe the peak at $\sim$1.85 \AA, which we presume contain overlapping Ta$_{2}$O$_{5}$ and GeO$_{2}$ peaks. As we alluded to earlier, we resolve the resolution problem by using EXAFS and \emph{ab-initio} MD data to supplement our results from the PDF analysis.
...
\begin{figure}[t]
\centering
\subfloat[\label{fig:label:waterfall}]{\includegraphics[width=0.5\textwidth]{./assets/waterfall.eps}}
\qquad
\subfloat[\label{fig:label:contour}]{\includegraphics[width=0.6\linewidth]{./assets/contour.eps}}
...
We take advantage of the chemical specificity of XAS to examine the Ta L-III edge to investigate the fine structure, or EXAFS centered around Ta atoms. The edge (Figure \ref{fig:label:xas_mu}) corresponds to dipole-allowed transitions 2p$_{3/2}\rightarrow$ 5d$_{5/2}$, 5d$_{3/2}$. There is no energy shift in the position of the white-line peak, which indicates that the formal valence of Ta is constant as Ge is incorporated into the amorphous structure. However, the height of the L-III edge decreases monotonically with Ge incorporation (Figure \ref{fig:label:xas_mu} inset). The height of the white-line peak directly correlates with d-orbital occupancy in 5d transition metals \cite{Qi1987}. Therefore, a decrease in the L-III edge peak suggests that Ge doping results in the increase of 5d occupation.

The behaviors of $k^{2}\chi(k)$ and its Fourier transform are plotted as a function of Ta cation composition in Figure \ref{fig:label:exafs_chik} and \ref{fig:label:exafs_chir}. Here, $\chi(R)$ is not corrected for the photoelectron phase shift. To gain a quantitative understanding of the EXAFS data, we fit four experimental EXAFS spectra to amorphous structural models of similar compositions derived from \emph{ab-initio} MD. Note, the calculated dielectric constants for our simulated structures show a similar trend to experimental values \cite{Naoi2018jap}, which indicates that our simulated structures possess local structures that are similar to that of the experimental samples (see Supplementary Material \cite{Supp}). The resulting fit parameters are tabulated in Table \ref{table:exafs2}, and the fitted EXAFS in $k$ and $R$-space are shown in Figure \ref{fig:exafs_fitted}. Our fits to pure Ta$_{2}$O$_{5}$ yield two Ta-O shells at 1.88 and 2.03 \AA \;(Table \ref{table:exafs2}), which shows the existence of two oxygen sites. These bond distances are close to our experimental results obtained from analysis of the PDF data (Figure \ref{fig:grfq}), although the PDF is able to resolve only a single Ta-O peak.
